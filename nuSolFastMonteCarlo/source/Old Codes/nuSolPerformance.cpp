...
double* johnTheInterpolator(std::string theFile){
  ...
  double theValues [3*(nLines-2)];// return the values as one long array
...
double* elliptical(double closest, double furthest){
  ...
  double e, eccentricity = (furthest/closest-1)/(furthest/closest+1);
  ...
  int cells = ceil(timeMax/kUnits::day);
  double theValues[cells];
  double xValues[cells];
  double yValues[cells];
  
  double x = furthest*0.999;
  double y = 0.001*furthest;
  double radius = sqrt(x*x+y*y);
  double angle = atan2(y,x);
  
  double vx = 0;
  double vy = sqrt(kUnits::G*kUnits::mSun* ( 2/furthest - 1/a ) ); // maximum velocity
  double v = sqrt(vx*vx+vy*vy);
  
  double accel = -kUnits::G*kUnits::mSun/furthest/furthest;
  
  cout << "I'm about to find the position/radii/etc for the ellipse.\n\n";
  for (int i = 0; i < 5; i++){
    cout << "day = " << i/kUnits::days << endl;
    int cellNum = ceil(i/kUnits::day);    
    cout << "r = " << radius/kUnits::solarRadii << endl;
    theValues[cellNum]=radius/kUnits::solarRadii;// in units of solar radii
    cout << "x = " << x/kUnits::solarRadii << endl;
    xValues[cellNum]=x/kUnits::solarRadii;// in units of solar radii
    cout << "y = " << y/kUnits::solarRadii << endl;
    yValues[cellNum]=y/kUnits::solarRadii;// in units of solar radii
    cout << "vx = " << vx/kUnits::solarRadii << endl;
    cout << "vy = " << vy/kUnits::solarRadii << endl;
    cout << "a = " << a/kUnits::solarRadii << endl;
    cout << "y = " << y/kUnits::solarRadii << endl<<endl;
    
    // Data updating
    x = x+cos(angle)*v*timeStep;
    y = y+sin(angle)*v*timeStep;
    radius = sqrt(x*x+y*y);
    angle = atan2(y,x);
    
    vx = vx+cos(angle)*accel*timeStep;
    vy = vy+sin(angle)*accel*timeStep;
    v = sqrt(vx*vx+vy*vy);

    accel = -kUnits::G*kUnits::mSun/(x*x+y*y);
  }
  ...
  double theReturns [3*(cells-1)];// return the values as one long array
  ...
  double angle = -M_PI+0.001;// starts at furthest approach, and this should do it.
  double radius = furthest;
  double dAngle = H/(radius)/(radius);// dr/dt
  double dRadius = p*e*sin(angle)/pow(1+e*cos(angle),2);// dTheta/dt
  //double timeStep = timeMax/1000;
  
  cout << "elliptical is about to iterate.\n";
    cout << "The current position is " << radius/kUnits::solarRadii << " solar radii." << endl;
    cout << "The current angle is " << angle << endl;
    cout << "The current dAngle " << dAngle << endl;
    cout << "The current dRadius " << dRadius/kUnits::solarRadii << " solar radii."  << endl << endl;
  for( int i = 0 ; i < intSteps; i++ ){
    
    // don't need ever step. Every 3 hours should be fine for interpolation
    int cellNum = ceil(i/kUnits::day);
    theValues[cellNum]=radius/kUnits::solarRadii;// in units of solar radii
    
    radius = radius+timeStep*dRadius;
    angle = angle+timeStep*dAngle;

    double dRdAngle = p*e/(1+e*cos(angle))/(1+e*cos(angle));//dr/dTheta
    double ddRdAngledAngle 
= p * (e*cos(radius) / (e*cos(radius)+1)/ (e*cos(radius)+1)
       +2*e*e*sin(radius)*sin(radius) / (e*cos(radius)+1) / (e*cos(radius)+1) / (e*cos(radius)+1)
 );// d^2 r/dTheta^2

    double ddAngle = -2*H*dRadius/radius/radius/radius;// d^2 Theta/dt^2
    double ddRadius = ddRdAngledAngle*dAngle*dAngle
      + dRdAngle*ddAngle;// d^2 r/dt^2
    
    dAngle = dAngle + ddAngle*timeStep;
    dRadius = dRadius+ ddRadius*timeStep;
...
double positionDays(double time, double* quadConst){
...
// minutes version
double positionMinutes(double time, double* quadConst){
  cout << "Position function has been called.\n";
  double currentRadius=0;
  int day = floor(time/kUnits::min);
  cout << "The time is " << (time/kUnits::min) << "hours in, and we are " 
       << day << " days in.\n";
...
double cosmicBackground (double radius, double inclination){
  ...
  */
  ...
  return 3.3*pow(10,4)*1.2; // 125% of the approximate proton rate to account for alphas 
...
void nuSolPerformance(){
  ...
  double time, radius, inclination,nuAccumulator,cosmicAccumulator,solarAccumulator=0;
  gRandom -> SetSeed(0);
  ...
  double cosmicAcceptance = 1/10000; // 4 nines
  ...
  /*
  double closest = 20*kUnits::solarRadii;
  double furthest = 1.1*kUnits::venusSunDistance;

  double a = (closest+furthest)/2;// semi-major axis
  timeLimit = 1;//2*M_PI*sqrt( pow(a,3) / ( kUnits::G*kUnits::mSun ) );
  double* quadraticConstants = elliptical(closest,furthest);
  cout << "elliptical  is done.\n\n";
    for (int i = 0;i<5;i++){
      cout << "Hour " << (i+1)*3 << " to " << (i+2)*3 << " has constants:\n";
      cout << "c = " << *(quadraticConstants+3*i);
      cout << "b = " << *(quadraticConstants+3*i+1);
      cout << "a = " << *(quadraticConstants+3*i+2) << "\n\n";
    }
  ...
  do{
    ...
    radius = (positionDays( time, quadraticConstants ));
    ...
    if(cosmicBack){   
      ...
      if(isCosmic){
	cosmicAccumulator = cosmicAccumulator - 1;
    ...
    if(solarBack){  
      ...
      if(isSolar){
	solarAccumulator = solarAccumulator - 1;
	...
	if(neutrinoOnly) neutrinoOnlyRadiusHistogram -> Fill( (radius / kUnits::solarRadii) );
