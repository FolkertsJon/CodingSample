/*
  ...
  - Jan 19: Exits
  Jan 19 2021 : Removed /steradian from the neutrino rates. (units in graph they were taken from 
                did not have them. Oops)
  Jan 22      : Found an error in the integrals. Should affect the rates somewhat significantly.
  Jan 28      : Moved accumulators and threshold declarations outside of the for loop. This should 
                mean that the 100 flights are like 100 consecutive flights rather than independent 
		ones. Should slightly improve rates, and make the model more realistic
...
double* elliptical (double closest, double furthest){
  ...
  cout << "There are " << sizeof(theValues) << " or " << cells << " values in the radius array.\n";


  cout << "I'm about to find the fit parameters for the ellipse.\n\n";
  ...
  for(long long int i=1; i<(cells-1);i++){ // when squaring they were getting over 2^31, so 64 bit we go
    // values found from calculating (a,b,c)
    // =inverse( (1,x(i-1),x^2(i-1); 1,x(i),x^2(i)); 1,x(i+1),x^2(i+1) )
    // (inner product) (c,b,a)^T
    ...
    if( false /*(i < 32773) && (i>32760)*/){
      cout << "a[" << i << "] is " << a << ". b[" << i << "] is " << b << ". c[" << i << "] is " << c  << endl;
      cout << "c = ( (i*i+i)*theValues[i-1] + (2-2*i*i)*theValues[i] + (i*i-i)*theValues[i+1] )/2. \ni = " 
	   << std::setprecision(15) << i << ". theValues[i-1] = " << (theValues[i-1]) << ". theValues[i] = " << (theValues[i]) << ". theValues[i+1] = " << (theValues[i+1]) << endl;
      double blarg0 = theValues[i-1];
      double blarg1 = theValues[i]; 
      double blarg2 = theValues[i+1];
      cout << "Doing the algebra on that gives me: " <<( (i*i+i)*blarg0 + (2-2*i*i)*blarg1 + (i*i-i)*blarg2 )/2 << "\n";
      cout << "But that's still wrong so what is i*i+i? i*i+i = " << (i*i+i) << " i*i-i = " << (i*i-i) << " and 2-2*i*i = " << (2-2*i*i) << endl;
  ...
  //cout << "c[max], b[max], and a[max], are " << theReturns[32798*3] << ", " << theReturns[32798*3+1] << ", " << theReturns[32799+2] << " respectively.\n\n";
...
double positionMinutes(double time, double* quadConst, double maxTime){
  ...
  if(time < (1*kUnits::min)){
  ...
  else if (time > (maxTime-kUnits::minute)){ // last day from earlier data
    // cout << "I made it here.\n";
    currentRadius = *(quadConst + 3*( day-2 ) )
    ...
    int out = day*kUnits::min/kUnits::day;
    //  cout << "The time is " <<  out << " days out of " << maxTime/kUnits::day << " days.\n";
...
double diffCrossSec(){
  ...
  double mksUnits = 17.2e-45*kUnits::cm*kUnits::cm; //FIAT! naturalUnits*conversion;
  return mksUnits;
}
...
double hepRate (double eMin, double eMax){
  ...
  double const1 = 26.31; // units are MeV^(-pow1-1) and so on
  double pow1 = 1.618;
  double constant = 700; // units are /MeV
  ...
  integral += const1 * ( pow(pow1Max,pow1+2) - pow(pow1Min,pow1+2) ) ; // Power integral
  integral += const2 * ( pow(pow2Max,pow2+2) - pow(pow2Min,pow2+2) ) ;
  integral += const3 * ( pow(pow3Max,pow3+2) - pow(pow3Min,pow3+2) ) ;
...
double kgDetector = 225;
double detectorVolume = kgDetector/detectorDensity;

double nProtons = 1+1+8;//6+2+6+1+6*6+4+6+3 ; // based on Polyvinyl Toluene (base for a scintillator) (CH2CH(C6H4CH3))
double massMolecules = (1+1+16)*kUnits::AMU;//(12+2+12+1+6*12+4+12+3)*kUnits::AMU;
...
// THE ITERATOR -------------------------------------------------------
int theIterator(TH1D neutrinoRadiusHistogram,
		TH1D sub35NeutrinoRadiusHistogram,
		TH1D neutrinoOnlyRadiusHistogram,
		TH1D sub35NeutrinoOnlyRadiusHistogram,
		TH1D cosmicBackRadiusHistogram,
		TH1D sub35CosmicBackRadiusHistogram,
		TH1D solarBackRadiusHistogram,
		TH1D sub35SolarBackRadiusHistogram,
		TH1D radiusHistogram,
...
int main(){
  ...
  double solarAcceptance = cosmicAcceptance/10000;// 4 nines better than cosmic
  //double timeLimit = number_of_lines(myFile)*kUnits::day; // soft cap

  // Derived Control Variables
  double histoLowerLimit = 1; // lower limit in solar radii
  double histoUpperLimit = 235;
  double histoBins = 1000;
  ...
  // Holder histos for each thread
  TH1D* neutrinoRadiusHistograms[nThreads];
  TH1D* sub35NeutrinoRadiusHistograms[nThreads];

  TH1D* neutrinoOnlyRadiusHistograms[nThreads];
  TH1D* sub35NeutrinoOnlyRadiusHistograms[nThreads];
  
  TH1D* cosmicBackRadiusHistograms[nThreads];
  TH1D* sub35CosmicBackRadiusHistograms[nThreads];

  TH1D* solarBackRadiusHistograms[nThreads];
  TH1D* sub35SolarBackRadiusHistograms[nThreads];

  TH1D* radiusHistograms[nThreads];
  ...
  double semi_majorAxis = (closest+furthest)/2;// semi-major axis
  timeLimit = ( 2*M_PI*sqrt(semi_majorAxis*semi_majorAxis*semi_majorAxis/( kUnits::G * kUnits::mSun ) ) );//2*M_PI*sqrt( pow(a,3) / ( kUnits::G*kUnits::mSun ) );
  ...
  /*for (int i = 0;i<100;i++){
    ...
    }*/ //Depricated checking code



  int threadNumbers[nThreads] = 0;
  for(int i=0; i < nThreads; i++){
    threadNumbers[i] = i;
  }

  int numberForEachThread = floor(nLoops/nThreads);
  int remainder = nLoops % nThreads;
  ...
  double nuAccumulator,nuAccumulator2,cosmicAccumulator,solarAccumulator=0;
  ...
  for(int i = 0; i < nLoops; i++ ){
	...
	std::cout << "Before flightPath, time is " << (time/kUnits::days) << " d, radius is " << (radius/kUnits::solarRadii) << " RSol, and inclination is " << inclination <<endl;
      ...
      radius = (positionMinutes (time, quadraticConstants, timeLimit )); // for file
	...
	break;
      ...
      if(cosmicBack){   
	double cosmicGeoAccept = 4*M_PI*kUnits::solarRadii*kUnits::solarRadii/(4/M_PI)/(radius*radius);// fraction of the sky that could be solar. We'll assume that we can isolate to 2 rSol (unrealistically good)
	double background = cosmicBackground(radius,1); // checking background
	double vetoedBack = background * cosmicAcceptance * cosmicGeoAccept;
	double a = timeStep * vetoedBack;//cosmicBackground(radius,1);
