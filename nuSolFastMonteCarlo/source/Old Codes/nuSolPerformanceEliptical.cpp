/*
  ...
  Last edit Dec 29 2020
  (added some couts for diagnosis jan 19 2021)
 */
...
double* elliptical (double closest, double furthest){
  ...
  const double timeStep = 1*kUnits::hour;  cout << "The specific angular momentum is " << H << endl;
...
// NEUTRINOS START ---------------------------------------------
double kgGallium = 25;
double galliumAtomNumber =kgGallium * 8.637e24;// 1 kg of gallium for now
...
int main(){
  ...
  //  Control Variables!
  double timeStep = 10*kUnits::sec; // time step for iterating
  ...
  for(int i = 0; i < 1; i++ ){
    ...
    do{
      ...
      radius = (positionDays( time, quadraticConstants, timeLimit ));
