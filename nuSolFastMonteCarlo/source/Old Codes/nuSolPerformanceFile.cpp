...
namespace kUnits
{
  ...
  const double G = 6.67430e-11; // units are all 1 in this system
...
double pepFlux(double eMin, double eMax){
  ...
  if ( (eMin < eDelta) && (eMax > eDelta)){
    return 1.3e8/(kUnits::cm)/(kUnits::cm)/(kUnits::s)/(M_PI*4);// per second per cm^2 per steradian
  }
  else{
    return 0;
  } 
...
double Be7Flux(double eMin, double eMax){
  ...
  if ( (eMin < eDelta1) && (eMax > eDelta1)){
    return 5e8/(kUnits::cm)/(kUnits::cm)/(kUnits::s)/(M_PI*4);// per second per cm^2 per steradian
  }
  else if ( (eMin < eDelta2) && (eMax > eDelta2)){
    return 4.1e9/(kUnits::cm)/(kUnits::cm)/(kUnits::s)/(M_PI*4);// per second per cm^2 per steradian
  }
  else{
    return 0;
  } 
...
double hepCrossSec(double eMin, double eMax){
  return 7.14e-46*kUnits::cm*kUnits::cm;
}

double kgGallium = 1000000;
...
int main(){
  ...
  for(int i = 0; i < nLoops; i++ ){
	...
	double b = timeStep * solarBackground(radius,1)* solarAcceptance * solarAcceptance;
